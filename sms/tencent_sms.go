...
// NewTencentSmsClient
func NewTencentSmsClient(conf TencentConfig) *TencentSmsClient {
	return &TencentSmsClient{Conf: conf}
}

// SendSms 发送验证码
func (s *TencentSmsClient) Send(mobile string, code string) (err error) {
	/* 必要步骤：
	...
	// TODO
	request.SmsSdkAppid = common.StringPtr(s.Conf.SdkAppID)
	...
	// TODO
	request.TemplateID = common.StringPtr(s.Conf.TemplateID)
	...
	// 打印返回的json字符串
	logrus.Debugf("sendSms json %s", string(b))
